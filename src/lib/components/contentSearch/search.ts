...
const COURSE_URL = courseEnv?.COURSE_URL;
...
export class NewQuizSearch extends ContentSearch<Link> {
    ...
    async getLinks(data: Record<string, any>[]) {
        return data.map<Link>((q) => {
            return {
                name: q.title,
                // new quiz url pattern: `https://canvas.auckland.ac.nz/courses/${COURSE_ID}/assignments/${q.id}`,
                url: `${COURSE_URL}/assignments/${q.id}`,
                type: InternalLinks.Quizzes,
                published: q.published,
            };
        });
