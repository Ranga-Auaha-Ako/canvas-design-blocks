...
class HeaderState implements SvelteState<HeaderData> {
    ...
    constructor(
        ...
        const existingTitle = existingTitleElement?.innerHTML ||
            existingTitleElement?.textContent?.trim();

        let state: HeaderData = {
            title: existingTitle || unsafeState?.title || HeaderState.defaultState.title,
            overview: "",
