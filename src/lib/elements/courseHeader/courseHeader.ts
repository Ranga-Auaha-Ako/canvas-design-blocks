...
export interface HeaderData {
  ...
  theme: HeaderTheme;
  textColor?: Colord;          // V2.14.7 renamed from color to be explicit
  backgroundColor?: Colord;    // explicitly for background
  level?: HeaderLevel;
  icon?: IconState;
}
...
class HeaderState implements SvelteState<HeaderData> {
  ...
  constructor(
    ...
    // Handle legacy color property from old headers
    if (unsafeState && 'color' in unsafeState && unsafeState.color) {
      // In Modern theme, the old color property was used for background
      if (theme === HeaderTheme.Modern) {
        state.backgroundColor = colord(unsafeState.color as any);
        state.textColor = colord(unsafeState.color as any).isDark()
          ? colord('#ffffff')
          : colord('#000000');
      } else {
        // For other themes, color might have been text color
        state.textColor = colord(unsafeState.color as any);
      }
    }
