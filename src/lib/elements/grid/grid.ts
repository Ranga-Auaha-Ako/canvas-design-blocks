...
export class Grid extends MceElement implements Readable<Row[]> {
    ...
    public static migrate(grid: Grid) {
        ...
        // SINCE alpha: Columns might have direct text descendants of innernode. This is no longer allowed, so we need to wrap them in a paragraph
    ...
    public bindEvents() {
        ...
        // clipboard clear styling
        this.editor.on("PastePreProcess", (e: { content: string }) => {
            const selection = this.editor.selection.getNode();
            if (!selection.closest(".cgb-col-inner")) return;

            // Strip HTML, keep only text
            const temp = document.createElement("div");
            temp.innerHTML = e.content;
            e.content = temp.textContent || "";
        });
    }
